...
class Mosaic_Net(nn.Module):
    def __init__(self):
        super(Mosaic_Net, self).__init__()
        ...
        self.conv1x1_1=nn.Conv2d(in_channels=512,out_channels=11,kernel_size=1)
    ...
    def forward(self, x):
        ...
        x=nn.Softmax(1)(x)
        batch_size=len(x)
        x=x.permute(0,2,3,1).contiguous().view(batch_size,81*11)
        x=x.view(batch_size,81,11)
        return x